...
export const show = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const surat = await getOutgoingLetterByRow(userId, rowNumber);
...
export const store = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    console.log('=== DIAGNOSTIC: LETTER SUBMIT REQUEST (OUTGOING) ===');
    console.log('[SuratKeluarSubmit] folder_id received:', folder_id);
    console.log('[SuratKeluarSubmit] folder_id type:', typeof folder_id);
