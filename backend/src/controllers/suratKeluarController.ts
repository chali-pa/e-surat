...
export const index = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch surat keluars', details: error.message || error.toString() });
  }
};
...
export const show = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const surat = await getOutgoingLetterRecordById(userId, suratId);
    ...
    res.status(500).json({ error: 'Failed to fetch surat keluar' });
  }
};
  ...
  try {
    ...
    if (!nomor_surat || !nama_penerima || !nama_surat || !tanggal_keluar || !tanggal_buat) {
      return res.status(400).json({
        ...
        message: 'Nomor surat, nama penerima, nama surat, tanggal keluar, and tanggal buat harus diisi',
        missing_fields: {
    ...
    if (req.file) {
      ...
      console.log('Uploading outgoing letter to user Drive:', cleanFileName);
    ...
    console.error('=== STORE SURAT KELUAR FAILED ===', error);
    if (isGoogleErrorInvalidGrant(error)) {
      return res.status(401).json({
        error_code: 'GOOGLE_RECONNECT_REQUIRED',
        error: 'Koneksi Google perlu diperbarui',
    ...
    res.status(500).json({
      error: error.message || 'Failed to create surat keluar',
      message: 'Gagal menyimpan surat keluar. Silakan coba lagi.',
...
export const update = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const oldSurat = await getOutgoingLetterRecordById(userId, rowNumber);
    if (!oldSurat) {
      return res.status(404).json({ error: 'Surat keluar not found' });
    }
    ...
    // The limit applies when the record being updated has a folder assigned
    // (either newly set by this request or already stored on the record).
    ...
    res.status(500).json({ error: 'Failed to update surat keluar' });
  }
};
...
export const destroy = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const surat = await getOutgoingLetterRecordById(userId, rowNumber);
    if (!surat) {
      return res.status(404).json({ error: 'Surat keluar not found' });
    }
...
export const preview = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    console.error('Preview surat keluar error:', error);
    if (isGoogleErrorInvalidGrant(error)) {
      return res.status(401).json({
        error_code: 'GOOGLE_RECONNECT_REQUIRED',
        error: 'Koneksi Google perlu diperbarui',
    ...
    res.status(500).json({ error: 'Failed to preview surat keluar' });
  }
};
