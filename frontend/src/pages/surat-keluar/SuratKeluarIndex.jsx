...
/**
 * Build the authenticated file URL for a given surat keluar ID.
 */
function getFileUrl(suratId, disposition = 'inline') {
  const token = localStorage.getItem('token')
  const base = `${API_BASE_URL}/api/surat-keluar/${suratId}/file`
...
export default function SuratKeluarIndex() {
  ...
  const fetchSurats = async () => {
    try {
      ...
      setSurats(response.data.surats || response.data.data || [])
    } catch (error) {
  ...
  const filteredSurats = surats.filter(surat =>
    ...
    surat.nama_penerima?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[#4B164C]">Surat Keluar</h1>
          <p className="mt-1 text-sm text-slate-500">Kelola surat keluar dengan tampilan ringkas dan bersih.</p>
        </div>
        <Link
        ...
        <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 text-amber-900">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-amber-700">Please reconnect your Google account to sync your outgoing letters with Google Drive &amp; Sheets.</p>
            </div>
          </div>
          <button
            type="button"
            ...
            <table className="w-full min-w-[700px]">
              <thead>
                <tr className="bg-[#FAF7FC]">
                  ...
                  <th className="py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-left">Penerima</th>
                  <th className="py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-left">Tgl Buat</th>
                  <th className="py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-left">Tgl Keluar</th>
                ...
                {filteredSurats.length === 0 ? (
                  ...
                  filteredSurats.map((surat, index) => (
                    <tr key={surat.id} className="hover:bg-purple-50/30 transition">
                      ...
                      <td className="py-3 px-4 text-sm text-slate-700">{surat.nama_penerima}</td>
                      <td className="py-3 px-4 text-sm text-slate-500">
                        {new Date(surat.tanggal_buat).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
                      </td>
                      <td className="py-3 px-4 text-sm text-slate-500">
                        {new Date(surat.tanggal_keluar).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
                      </td>
      ...
      {deleteModal.show && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            <h3 className="text-lg font-semibold text-gray-900 text-center mb-1">Hapus Surat Keluar?</h3>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-2.5 px-4 rounded-xl bg-red-600 text-white font-semibold hover:bg-red-700 transition text-sm"
              >
                Hapus
              </button>
            </div>
      ...
      <DocumentPreviewModal
        ...
        apiEndpoint={previewModal.surat ? `/api/surat-keluar/${previewModal.surat.id}/file` : ''}
      />
    </div>
  )
}
