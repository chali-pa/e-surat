...
use App\Data\SuratKeluarRecord;
...
class GoogleSheetKeluarService
{
    private ?Sheets $sheets = null;

    private bool $headersEnsured = false;

    /** @var array<int, SuratKeluarRecord>|null */
    ...
    {
        // Use user's sheet keluar ID if available, otherwise fall back to global config
        return $this->user && $this->user->google_sheet_keluar_id 
            ? $this->user->google_sheet_keluar_id 
            : (string) config('google.sheet_keluar_id');
    }
    ...
    private function getSheetName(): string
    {
        return $this->user && $this->user->google_sheet_keluar_id 
            ...
            : config('google.sheet_keluar_name', 'Sheet1');
    }
    ...
    /**
     * @return Collection<int, SuratKeluarRecord>
     */
    public function all(): Collection
    {
        return collect($this->loadRows())
            ->sortByDesc(fn (SuratKeluarRecord $surat) => $surat->created_at?->timestamp ?? 0)
            ->values();
    }
    ...
    public function search(string $query): Collection
    {
        ...
        return $this->all()->filter(function (SuratKeluarRecord $surat) use ($needle) {
    ...
    public function find(int $id): ?SuratKeluarRecord
    {
     ...
     */
    public function insert(array $data, ?string $driveFileId = null, ?string $driveLink = null): SuratKeluarRecord
    {
        ...
        $now = now();
        $surat = new SuratKeluarRecord(
            id: $this->nextId(),
            nomor_surat: (string) ($data['nomor_surat'] ?? ''),
            tanggal_keluar: (string) ($data['tanggal_keluar'] ?? ''),
            tanggal_buat: (string) ($data['tanggal_buat'] ?? ''),
            nama_penerima: (string) ($data['nama_penerima'] ?? ''),
     ...
     * @param  array<string, mixed>  $data
     */
    public function update(int $id, array $data, ?string $driveFileId = null, ?string $driveLink = null): ?SuratKeluarRecord
    {
        if (! $this->isConfigured()) {
            throw new \RuntimeException('Google Sheets belum dikonfigurasi.');
        }
        ...
        $updated = new SuratKeluarRecord(
            id: $existing->id,
            nomor_surat: (string) ($data['nomor_surat'] ?? $existing->nomor_surat),
            tanggal_keluar: (string) ($data['tanggal_keluar'] ?? $existing->tanggal_keluar),
            tanggal_buat: (string) ($data['tanggal_buat'] ?? $existing->tanggal_buat),
            nama_penerima: (string) ($data['nama_penerima'] ?? $existing->nama_penerima),
    ...
    /**
     * @return array<int, SuratKeluarRecord>
     */
    private function loadRows(): array
    {
        ...
        try {
            ...
            foreach ($response->getValues() ?? [] as $row) {
                $record = SuratKeluarRecord::fromSheetRow($row);
    ...
    private function getSheetNumericId(): int
    {
        ...
        $sheetName = config('google.sheet_keluar_name', 'Sheet1');
    ...
    {
        return config('google.sheet_keluar_name', 'Sheet1') . '!' . $cells;
    }
