  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.response?.data?.error_code === 'GOOGLE_RECONNECT_REQUIRED') {
        ...
        setErrors({ general: 'Failed to update surat keluar. Please try again.' })
      }
    } finally {
      setSubmitting(false)
    }
    ...
    <main className="min-h-screen p-4 md:p-8 bg-[#F9FAFB]">
      <div className="max-w-3xl mx-auto">
        ...
        <header className="bg-gradient-to-r from-[#4B164C] to-[#DD88CF] shadow-md p-5 flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-center rounded-2xl mb-8">
          <h2 className="text-xl font-bold text-white">Edit Surat Keluar</h2>
          <Link to="/surat-keluar" className="text-white hover:text-gray-200 transition flex items-center gap-2 font-medium">
        ...
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-5 sm:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full border rounded-xl px-4 py-3 outline-none transition ${errors.nama_penerima ? 'border-red-400' : 'border-gray-200'}`}
                  required
                />
                {errors.nama_penerima && <p className="text-red-500 text-xs mt-1">{errors.nama_penerima}</p>}
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Tanggal Keluar</label>
                <input
                  type="date"
                  value={formData.tanggal_keluar}
                  onChange={(e) => setFormData({ ...formData, tanggal_keluar: e.target.value })}
                  className={`w-full border rounded-xl px-4 py-3 outline-none transition ${errors.tanggal_keluar ? 'border-red-400' : 'border-gray-200'}`}
                  required
                />
                {errors.tanggal_keluar && <p className="text-red-500 text-xs mt-1">{errors.tanggal_keluar}</p>}
              </div>
              <div>
            ...
            <div className="flex justify-end pt-4">
              ...
              >
                {submitting ? 'Memproses...' : 'Update Surat Keluar'}
              </button>
            </div>
          </form>
        </div>
